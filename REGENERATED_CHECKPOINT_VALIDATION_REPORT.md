...
This report is a supportive reproducibility and integrity check of Tanisha's thesis framework using regenerated ImageNet-initialized Stage 1 checkpoints. It is not a competing experiment and should not be framed as "my result versus Tanisha's result." The purpose is to check whether the same comparison framework produces the same direction of evidence, and to clearly flag any places where my regenerated run differs from Tanisha's reported comparison.

This is not an exact artifact reproduction of Tanisha's reported result because the original checkpoint LFS objects were not available locally. The run below uses regenerated checkpoints and the currently available 2020 data/artifacts.

## How to Read This Report

The numbers below should be read as a sensitivity check under regenerated artifacts, not as an alternative thesis result. Tanisha's reported result remains the primary result for her thesis claim because it uses her frozen pipeline and artifacts.

The most important difference is alert volume. Tanisha's saved 2020 run produces about 89.53 Stage 2 alerts/event-day. My regenerated-checkpoint run produces about 25.23 Stage 2 alerts/event-day using the newly calibrated regenerated-pipeline Stage 2 threshold, `0.8117221967059977`. That difference is expected because my run uses a smaller available 2020 data universe and regenerated Stage 1 checkpoints, which change the Stage 2 score distribution.
...
Because the alert budgets differ so much, the regenerated-checkpoint run should not be used as an exact numerical reproduction of Tanisha's point 1. It is still valuable as an integrity check: if the same framework gives the same direction, that supports the claim; if it gives a different direction, that difference must be highlighted rather than hidden. In this run, the matched-volume WSTS comparison is different from Tanisha's reported direction on precision, recall, and true-positive alerts, while the alert-fatigue and ranking-quality comparisons point in the same direction as her thesis.
...
I checked the public repo scripts that are shared between Tanisha's documented 2020 workflow and my run:
...
There were no local code diffs in those shared scripts. The documented `--exclude-empty-current-fire` flag was also tested on my available 2020 data and produced the same candidate count, so it does not explain the lower true-positive count.

After Tanisha's exact `26_compare_matched_alert_volume.py` and `27_summarize_matched_alert_volume.py` were shared into `Specifications/`, I compared them with my local implementation and ran Tanisha's comparison script on my scored candidates. Her script could not run standalone because the helper module `wildfire_alert.evaluation.matched_alerts` is not present in the repo, so I used a compatibility implementation matching the API and metric definitions imported by her script.

Tanisha's script on my scored candidates produced the same matched-volume precision, recall, and true-positive discrepancy:
...
Audit conclusion: different result direction for the main matched-volume claim. My run confirms the matched-volume setup and shows Stage 2 catches more event-days than WSTS, but it does not reproduce Tanisha's higher precision or higher true-positive alert count under the smaller regenerated-checkpoint alert budget. This should be reported openly as an artifact-sensitive discrepancy, not hidden and not framed as a rival thesis result.

Why the alert volume is lower: the runs use different frozen Stage 2 thresholds because mine was recalibrated from regenerated 2019 OOF Stage 2 scores: Tanisha uses `0.7777810642240797`, while my consistent regenerated pipeline uses `0.8117221967059977` and a smaller 2020 candidate universe. Tanisha's saved 2020 rolling run has 201 events, 2,184 samples, and a naive 4 km volume of 2,589.46 alerts/event-day. My available 2020 run has 50 events, 342 samples, and a naive 4 km volume of 1,156.02 alerts/event-day. Therefore, the regenerated-pipeline Stage 2 threshold produces about 89.53 alerts/event-day in Tanisha's saved run but about 25.23 alerts/event-day in mine.
...
| Metric | Tanisha Stage 2 | Tanisha Naive 4 km | My Stage 2 | My Naive 4 km |
|---|---:|---:|---:|---:|
| Alerts/event-day | 89.53 | 2,589.46 | 25.23 | 1,156.02 |
| Precision | 0.1747 | 0.0396 | 0.1215 | 0.0296 |
| Alert reduction vs naive | 97.01% | - | 98.11% | - |

Supportive validation conclusion: agrees with Tanisha's thesis direction. The regenerated-checkpoint run confirms the alert-fatigue claim: Stage 2 produces dramatically fewer alerts than naive 4 km flooding and has much higher precision.
...
| Metric | Tanisha Stage 2 | Tanisha WSTS/Stage 1 | Tanisha Distance | My Stage 2 | My WSTS/Stage 1 | My Distance |
|---|---:|---:|---:|---:|---:|---:|
...
Supportive validation conclusion: agrees with Tanisha's thesis direction. The regenerated-checkpoint run confirms that Stage 2 ranks candidates better than WSTS/Stage 1 and distance-only baselines by AP and AUC.

## Overall Validator Summary

| Thesis point | Reproducibility / integrity result |
|---|---|
| 1. Stage 2 beats WSTS-as-alerter at matched alert volume | Not reproduced in full. Stage 2 improves event-day catch rate in my run, but WSTS has better precision, recall, and true-positive count. This must be highlighted as a discrepancy. |
...
- `outputs/stage2/matched_volume_imagenet_noamp_retrained_stage2_tanisha_script/`
