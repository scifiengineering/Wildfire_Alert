# original thesis Follow-up Figures

This folder contains only the new figures for the original thesis follow-up request. It does not duplicate or replace the existing final-pipeline figures in `outputs/figures/final_pipeline/`.
...
- Purpose: shows the highest-scoring regenerated-checkpoint sampled alert across five unique fire events and summarizes how added feature groups contribute to each alert score.
- Use for: the request to use the SHAP file to explain top alert locations across five unique fire events.
