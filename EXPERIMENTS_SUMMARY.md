...
This document summarizes the reproduction and validation work done around the original thesis matched-volume wildfire alerting experiment. It focuses on the important differences we found, why the numbers changed between runs, and how the corrected experiments support the same thesis argument.

## Core Thesis Argument

the original thesis alerting argument is not that Stage 2 produces the exact same number under every checkpoint or machine. The argument is:
...
|---|---|---|
| original-checkpoint rerun | `Specifications/stage1_fold_0/1/2_best.pt` | Closest local rerun using the original Stage 1 weights |
| Regenerated-checkpoint support run | Locally retrained ImageNet-initialized Stage 1 checkpoints | Independent support experiment without the original checkpoint weights |
...
| original thesis reported | WSTS | 89.52 | 0.2166 | 0.1834 | 104,361 | 0.4908 | not available |
| original-checkpoint rerun | Stage 2 | 114.29 | 0.2521 | 0.2725 | 155,098 | 0.7334 | 0.6122 |
| original-checkpoint rerun | WSTS | 114.30 | 0.2062 | 0.2229 | 126,837 | 0.5304 | 0.4428 |
...
These are not identical numeric results, but they support the same claim. The value of the extra runs is that the conclusion survives both the original checkpoint files and independently regenerated checkpoints.
...
original thesis also requested a smaller follow-up around explainability, coordinate conversion, and whether the added Stage 2 features matter. The SHAP and coordinate-conversion files were already present in the repository:
...
The report trained two-fold LightGBM variants on the 2019 `r2km_fair` candidate tables. The accepted full-feature models are the reference. The no-extra-feature baseline kept only Stage 1 probability/threshold-mask features plus current-fire distance/geometry features.
...
The follow-up figures should be additions, not replacements for the existing `outputs/figures/final_pipeline/` set. The initial review rejected the initial top-5 SHAP scatter plot generated during this follow-up because it was not presentation-worthy: the x-axis labels were crowded, annotations collided with the title, repeated event labels dominated the plot, and the visual did not communicate a clean claim.

The follow-up figure folder contains only the three new figures for the request. The detailed follow-up report includes a figure guide explaining what each diagram shows and why it matters:
...
For the thesis/report, these should be considered supplemental follow-up figures beside the existing `outputs/figures/final_pipeline/` set, not replacements.

## Catch-Rate Definitions

the requested clarification is useful because the two catch-rate definitions answer different questions.
...
|---|---|---:|---:|
| original-checkpoint rerun | Stage 2 | 0.7334 | 0.6122 |
| original-checkpoint rerun | WSTS | 0.5304 | 0.4428 |
...
## Anything Not Supporting the Main Argument?

In the full-2020 thesis-facing results summarized here, the analysis did not find a result that reverses the main argument:

- the reported thesis result supports the argument.
- The local rerun using the original thesis Stage 1 checkpoints supports the argument.
...
- Original-checkpoint full-2020 matched-volume artifacts.
- Original-checkpoint full-2020 dual-catch matched-volume artifacts.
